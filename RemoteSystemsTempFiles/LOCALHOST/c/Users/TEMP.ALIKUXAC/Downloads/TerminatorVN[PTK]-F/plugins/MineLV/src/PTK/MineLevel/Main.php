    ...
    public function onBreak(BlockBreakEvent $ev){
        ...
        if($block->getId() === Block::DIAMOND_BLOCK or $block->getId() === Block::EMERALD_BLOCK or $block->getId() === Block::REDSTONE_BLOCK or $block->getId() === Block::GOLD_BLOCK or $block->getId() === Block::IRON_BLOCK or $block->getId() === Block::LAPIS_BLOCK){
            ...
            if($this->getCurrentExp($p) >= $this->getLevelUpExp($p)){
                ...
                $p->getInventory()->addItem(Item::get(131,2,1));
                $p->sendMessage("§b[§cT§aᗰ§6K§b-§9LEVEL§b]§e•»§2 Bạn Đã Nhận Được 1 Chìa Khóa Hiếm!!");
            }
        }
        ...
        switch($command->getName()){
            case "ilevel":
                ...
                if (isset($args[0])){
                    ...
                    $s->sendMessage("§b[§cT§aᗰ§6K§b-§9LEVEL§b]§e•»§c Người Chơi Không Tồn Tại!");
                }}
                return true;
            case "setilevel":
                if (count($args) < 0) {
                    $s->sendMessage("§b[§cT§aᗰ§6K§b-§9LEVEL§b]§e•»§6 Xin Hãy Sử Dụng Lệnh: /setilevel (Tên Người Chơi) [Số Cấp Độ]");
                    
                }
                if (!is_numeric($args[0])) {
                    $s->sendMessage("§b[§cT§aᗰ§6K§b-§9LEVEL§b]§e•»§6 Số Cấp Độ Phải Là Số!");
                }
                if (isset($args[1])) {
